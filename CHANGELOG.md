...
### Added
- **Backend Phase 2 — missing CRUD endpoints + server-side forecast calculator.**
  - `GET /api/yield-forecasts?zone_id=` computes the Seasonal Forecast on demand. Pure implementation in `src/lib/forecast.ts`: buckets harvest history by month, converts totals to kg/ha using the zone's area, averages the trailing 3 months, and projects 6 months forward. Confidence scales 0..1 with months of history (capped at 12). Stateless — no `yield_forecasts` writes yet. Covered by 5 unit tests.
  - `PATCH` / `DELETE /api/plantation-batches/[id]` close the gap where the zone-detail Batches tab could only create rows. Both handlers verify `batch.user_id === auth.uid()` before mutating.
  - `GET` and `PATCH /api/profile` replace the direct Supabase calls the profile and dashboard pages make. `GET` lazily creates the profile row on first call so fresh signups don't need manual setup.
  - `GET /api/farms/[id]/stats` aggregates `zones_count`, `trees_count`, `total_area_ha`, `harvests_count`, `total_harvest_quantity`, `last_harvest_date`, and `yield_per_hectare` in one round-trip. Replaces the three-query dance the dashboard home and profile page used to do against Supabase directly.
  - `GET /api/i18n-strings` returns translation overrides through the API (unauthenticated, matches the RLS `read_all` policy). Updates the client `getI18nStrings()` helper to fetch via this route instead of calling Supabase directly.
  - **Zone detail page** now consumes the forecast API: replaces the direct `supabase.from('yield_forecasts').select(...)` with `fetch('/api/yield-forecasts?zone_id=...')`. Plantation-batch rows now render **Edit** and **Delete** buttons with a confirmation dialog, both paths wired to the new `[id]` endpoints with translated toasts.
